...
description: "AnalOS works best with your own API keys!"
---

To use AnalOS, you need to configure an LLM provider. You have two options:

1. **Use models locally** - Connect AnalOS to models running on your machine
2. **Bring your own API keys** - Use cloud providers like ChatGPT, Gemini, or Claude

Navigate to `chrome://analos/settings` or click the **Settings** icon on the new tab page to configure your LLM provider.

<Note>
  **Our recommendation:** AnalOS works best with **Gemini 2.5 Flash**. We highly recommend getting an API key from Google AI Studio for Gemini 2.5 Flash. **Google AI Studio gives free access to Gemini 2.5 Flash models for up to 20 requests per minute** - perfect for getting started without any costs.
</Note>
...
<AccordionGroup>
  <Accordion title="🔒 Privacy - You control your data">
    Your API keys are stored locally and encrypted. Requests go directly from your browser to the provider - AnalOS servers never see your data or keys. With local models, your data never even leaves your machine.
  </Accordion>
  <Accordion title="⚡ Rate limits - Avoid usage restrictions">
    AnalOS has heavy rate limits on the default shared models. For the smoothest experience without interruptions, bring your own API keys or run models locally. Use AnalOS as much as you want without hitting limits.
  </Accordion>
</AccordionGroup>

## Option #1: Bring your own keys to ChatGPT, Gemini, Claude! (Recommended)

Connect to powerful cloud models using your own API keys. AnalOS works very well with Gemini 2.5 Flash - use it for the highest performance, fastest speed, and most stable experience.
...
<Warning>
  Many models like Deepseek-R1 via Ollama don't support tool calls and you'll get `400` errors when trying to use them with AnalOS. Stick with the recommended models above for the best experience.
</Warning>
...
Once you've configured your LLM provider, explore what AnalOS can do:
...
- Use **Teach Mode** to create repeatable workflows—show the AnalOS agent your workflow once, and it will learn and repeat it reliably.