...
+SidePanelService::AnalosToggleSidePanelForTab(
+    const Extension& extension,
+    content::BrowserContext* context,
+    int tab_id,
+    bool include_incognito_information,
+    std::optional<bool> desired_state) {
+  LOG(INFO) << "analos: AnalosToggleSidePanelForTab called for tab_id="
...
+      !window || !web_contents) {
+    LOG(WARNING) << "analos: Tab not found for tab_id=" << tab_id;
...
+  if (!browser_window) {
+    LOG(WARNING) << "analos: No browser window for tab_id=" << tab_id;
...
+  }
+
+  LOG(INFO) << "analos: has_contextual_options=" << has_contextual_options
+            << " for tab_id=" << tab_id;
+
...
+      LOG(WARNING) << "analos: No side panel path configured for extension="
+                   << extension.id();
+      return base::unexpected(
...
+    }
+
+    LOG(INFO) << "analos: Auto-registering contextual panel for tab_id="
...
+    LOG(WARNING) << "analos: Side panel is disabled for tab_id=" << tab_id;
+    return base::unexpected(
+        base::StringPrintf("Side panel is disabled for tabId: %d", tab_id));
+  }
+
+  // Toggle the contextual panel.
+  LOG(INFO) << "analos: Calling ToggleContextualExtensionSidePanel for tab_id="
+            << tab_id;
...
+
+  LOG(INFO) << "analos: Toggle result: is_now_open=" << is_now_open
+            << " for tab_id=" << tab_id;
+
+  return is_now_open;
+}
+
+base::expected<bool, std::string>
+SidePanelService::AnalosIsSidePanelOpenForTab(
+    const Extension& extension,
+    content::BrowserContext* context,
+    int tab_id,
+    bool include_incognito_information) {
+  LOG(INFO) << "analos: AnalosIsSidePanelOpenForTab called for tab_id="
+            << tab_id << ", extension=" << extension.id();
+
+  // Find the tab.
+  WindowController* window = nullptr;
...
+  api::side_panel::PanelOptions current_options = GetOptions(extension, tab_id);
+  if (!current_options.enabled.value_or(true)) {
+    LOG(INFO) << "analos: Panel is disabled for tab_id=" << tab_id
+              << ", returning false";
+    return false;
+  }
+
...
+
+  LOG(INFO) << "analos: IsOpen result: is_open=" << is_open
+            << " for tab_id=" << tab_id;
+
+  return is_open;
+}
