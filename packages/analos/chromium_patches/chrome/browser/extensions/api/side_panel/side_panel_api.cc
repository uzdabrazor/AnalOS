...
+SidePanelAnalosToggleFunction::RunFunction() {
+  EXTENSION_FUNCTION_VALIDATE(extension());
+
+  std::optional<api::side_panel::AnalosToggle::Params> params =
+      api::side_panel::AnalosToggle::Params::Create(args());
...
+      service->AnalosToggleSidePanelForTab(
...
+  }
+
+  api::side_panel::AnalosToggleResult result;
+  result.opened = toggle_result.value();
+  return RespondNow(WithArguments(result.ToValue()));
+}
+
+ExtensionFunction::ResponseAction
+SidePanelAnalosIsOpenFunction::RunFunction() {
+  EXTENSION_FUNCTION_VALIDATE(extension());
+
+  std::optional<api::side_panel::AnalosIsOpen::Params> params =
+      api::side_panel::AnalosIsOpen::Params::Create(args());
+  EXTENSION_FUNCTION_VALIDATE(params);
+
+  SidePanelService* service = GetService();
+  base::expected<bool, std::string> is_open_result =
+      service->AnalosIsSidePanelOpenForTab(
+          *extension(), browser_context(), params->options.tab_id,
