...
+    LOG(INFO) << "analos: ExtractPageContent - tree is empty";
+    return items;
+  }
+
+  LOG(INFO) << "analos: ExtractPageContent - processing " << tree_update.nodes.size() << " nodes";
+
...
+
+  LOG(INFO) << "analos: ExtractPageContent - extracted " << items.size() << " items";
+
+  return items;
+}
+
