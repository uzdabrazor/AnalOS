...
+uint32_t AnalOSGetInteractiveSnapshotFunction::next_snapshot_id_ = 1;
+
+// Constructor and destructor implementations
+AnalOSGetInteractiveSnapshotFunction::AnalOSGetInteractiveSnapshotFunction() = default;
+AnalOSGetInteractiveSnapshotFunction::~AnalOSGetInteractiveSnapshotFunction() = default;
+
+ExtensionFunction::ResponseAction AnalOSGetAccessibilityTreeFunction::Run() {
...
+      base::BindOnce(
+          &AnalOSGetAccessibilityTreeFunction::OnAccessibilityTreeReceived,
+          this),
...
+}
+
+void AnalOSGetAccessibilityTreeFunction::OnAccessibilityTreeReceived(
...
+}
+
+// Implementation of AnalOSGetInteractiveSnapshotFunction
+
+ExtensionFunction::ResponseAction AnalOSGetInteractiveSnapshotFunction::Run() {
...
+    LOG(WARNING) << "[analos] Frame not stable for AX snapshot - skipping";
...
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(
+          &AnalOSGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived,
+          this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties |
+                 ui::AXMode::kInlineTextBoxes),
+      /* max_nodes= */ 0,  // No limit
...
+}
+
+void AnalOSGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  // Double-check frame is still valid before processing
+  if (!web_contents_) {
+    LOG(WARNING) << "[analos] WebContents gone during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+    LOG(WARNING) << "[analos] Frame became unstable during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+      web_contents_,
+      base::BindOnce(
+          &AnalOSGetInteractiveSnapshotFunction::OnSnapshotProcessed,
+          base::WrapRefCounted(this)));
+}
+
+void AnalOSGetInteractiveSnapshotFunction::OnSnapshotProcessed(
+    SnapshotProcessingResult result) {
+  Respond(ArgumentList(
+      browser_os::GetInteractiveSnapshot::Results::Create(result.snapshot)));
+}
+
+// Implementation of AnalOSClickFunction
+
+ExtensionFunction::ResponseAction AnalOSClickFunction::Run() {
...
+}
+
+// Implementation of AnalOSInputTextFunction
+
+ExtensionFunction::ResponseAction AnalOSInputTextFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[analos] InputText: Starting input for nodeId: " << params->node_id;
+  
...
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[analos] InputText: No change detected after typing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of AnalOSClearFunction
+
+ExtensionFunction::ResponseAction AnalOSClearFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[analos] Clear: Clearing field for nodeId: " << params->node_id;
+  
+  // Use ClearWithDetection which handles focus and clearing
+  bool change_detected = ClearWithDetection(web_contents, node_info);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[analos] Clear: No change detected after clearing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of AnalOSGetPageLoadStatusFunction
+
+ExtensionFunction::ResponseAction AnalOSGetPageLoadStatusFunction::Run() {
...
+}
+
+// Implementation of AnalOSScrollUpFunction
+
+ExtensionFunction::ResponseAction AnalOSScrollUpFunction::Run() {
...
+}
+
+// Implementation of AnalOSScrollDownFunction
+
+ExtensionFunction::ResponseAction AnalOSScrollDownFunction::Run() {
...
+}
+
+// Implementation of AnalOSScrollToNodeFunction
+
+ExtensionFunction::ResponseAction AnalOSScrollToNodeFunction::Run() {
...
+}
+
+// Implementation of AnalOSSendKeysFunction
+
+ExtensionFunction::ResponseAction AnalOSSendKeysFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[analos] SendKeys: Sending key '" << params->key << "'";
+  
+  // Send the key with change detection
+  bool change_detected = KeyPressWithDetection(web_contents, params->key);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[analos] SendKeys: No change detected after key press";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of AnalOSCaptureScreenshotFunction
+
+AnalOSCaptureScreenshotFunction::AnalOSCaptureScreenshotFunction() = default;
+AnalOSCaptureScreenshotFunction::~AnalOSCaptureScreenshotFunction() = default;
+
+ExtensionFunction::ResponseAction AnalOSCaptureScreenshotFunction::Run() {
...
+    LOG(INFO) << "[analos] CaptureScreenshot: Using exact dimensions: "
...
+      LOG(INFO) << "[analos] CaptureScreenshot: Using thumbnail size: " << max_dimension 
...
+      LOG(INFO) << "[analos] CaptureScreenshot: Using viewport size: " << max_dimension;
+    }
+    
...
+}
+
+void AnalOSCaptureScreenshotFunction::DrawHighlightsAndCapture() {
...
+      LOG(INFO) << "[analos] Drawing highlights for screenshot with " 
+                << tab_it->second.size() << " interactive elements";
+      ShowHighlights(web_contents_, tab_it->second, true /* show_labels */);
+    } else {
+      LOG(INFO) << "[analos] No snapshot data available for highlighting";
+    }
+    
...
+        FROM_HERE,
+        base::BindOnce(&AnalOSCaptureScreenshotFunction::CaptureScreenshotNow,
+                       base::WrapRefCounted(this)),
...
+}
+
+void AnalOSCaptureScreenshotFunction::CaptureScreenshotNow() {
+  if (!web_contents_) {
...
+      target_size_,
+      base::BindOnce(&AnalOSCaptureScreenshotFunction::OnScreenshotCaptured,
+                     this));
+}
+
+void AnalOSCaptureScreenshotFunction::OnScreenshotCaptured(
...
+}
+
+// AnalOSGetSnapshotFunction implementation
+ExtensionFunction::ResponseAction AnalOSGetSnapshotFunction::Run() {
...
+  // Request accessibility tree snapshot
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(&AnalOSGetSnapshotFunction::OnAccessibilityTreeReceived,
+                     this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties),
+      /* max_nodes= */ 0,  // No limit
+      /* timeout= */ base::TimeDelta(),
...
+}
+
+void AnalOSGetSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  if (!has_callback()) {
+    return;
+  }
...
+}
+
+// AnalOSGetPrefFunction
+ExtensionFunction::ResponseAction AnalOSGetPrefFunction::Run() {
...
+}
+
+// AnalOSSetPrefFunction
+ExtensionFunction::ResponseAction AnalOSSetPrefFunction::Run() {
+  std::optional<browser_os::SetPref::Params> params =
+      browser_os::SetPref::Params::Create(args());
+  EXTENSION_FUNCTION_VALIDATE(params);
+
+  // Security: only allow modifying analos.* prefs
+  if (!params->name.starts_with("analos.")) {
+    return RespondNow(Error("Only analos.* preferences can be modified"));
+  }
+
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* prefs = FindPrefService(params->name, profile);
...
+}
+
+// AnalOSGetAllPrefsFunction
+ExtensionFunction::ResponseAction AnalOSGetAllPrefsFunction::Run() {
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* profile_prefs = profile->GetPrefs();
+  PrefService* local_state = g_browser_process->local_state();
+
+  // Build a combined analos prefs dict from both sources
+  base::Value::Dict combined_analos;
+
+  // Lambda to merge analos prefs from a PrefService
...
+
+    // Look for "analos" key in the top-level dict
+    const base::Value* analos_value = pref_dict.Find("analos");
+    if (analos_value && analos_value->is_dict()) {
+      // Merge this analos dict into combined
+      combined_analos.Merge(analos_value->GetDict().Clone());
+      LOG(INFO) << "[analos] GetAllPrefs: Found analos.* prefs in " << source_name;
+    }
+  };
+
...
+
+  // Create single PrefObject with the entire analos dict
+  std::vector<browser_os::PrefObject> pref_objects;
+  browser_os::PrefObject pref_obj;
+  pref_obj.key = "analos";
+  pref_obj.type = "dictionary";
+  pref_obj.value = base::Value(std::move(combined_analos));
...
+}
+
+// AnalOSLogMetricFunction
+ExtensionFunction::ResponseAction AnalOSLogMetricFunction::Run() {
...
+    
+    analos_metrics::AnalOSMetrics::Log(prefixed_event, std::move(properties));
+  } else {
+    // No properties, just log with extension ID
+    analos_metrics::AnalOSMetrics::Log(prefixed_event, {
...
+}
+
+// AnalOSGetVersionNumberFunction
+ExtensionFunction::ResponseAction AnalOSGetVersionNumberFunction::Run() {
...
+}
+
+// AnalOSGetAnalosVersionNumberFunction
+ExtensionFunction::ResponseAction AnalOSGetAnalosVersionNumberFunction::Run() {
+  std::string version = std::string(version_info::GetAnalOSVersionNumber());
+
+  return RespondNow(ArgumentList(
+      browser_os::GetAnalosVersionNumber::Results::Create(version)));
+}
+
+// AnalOSExecuteJavaScriptFunction
+ExtensionFunction::ResponseAction AnalOSExecuteJavaScriptFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[analos] ExecuteJavaScript: Executing code in tab " << tab_info->tab_id;
+  
...
+      js_code,
+      base::BindOnce(&AnalOSExecuteJavaScriptFunction::OnJavaScriptExecuted,
+                     this),
...
+}
+
+void AnalOSExecuteJavaScriptFunction::OnJavaScriptExecuted(base::Value result) {
+  LOG(INFO) << "[analos] ExecuteJavaScript: Execution completed";
+
+  if (result.is_none()) {
...
+}
+
+// Implementation of AnalOSClickCoordinatesFunction
+ExtensionFunction::ResponseAction AnalOSClickCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[analos] ClickCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
...
+  
+  LOG(INFO) << "[analos] ClickCoordinates: Clicking at (" 
+            << params->x << ", " << params->y << ")";
+  
...
+  
+  LOG(INFO) << "[analos] ClickCoordinates: Result = " 
...
+}
+
+// Implementation of AnalOSTypeAtCoordinatesFunction  
+ExtensionFunction::ResponseAction AnalOSTypeAtCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[analos] TypeAtCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
+    response.success = false;
+    return RespondNow(ArgumentList(
...
+  gfx::PointF click_point(params->x, params->y);
+  
+  LOG(INFO) << "[analos] TypeAtCoordinates: Clicking at (" 
...
+  response.success = success;
+  
+  LOG(INFO) << "[analos] TypeAtCoordinates: Result = " 
