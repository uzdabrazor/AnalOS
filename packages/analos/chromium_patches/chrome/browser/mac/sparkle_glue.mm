...
+  const char* kBaseURL = "https://cdn.uzdabrazor.com/";
+
...
+
+// Forward declare internal SparkleGlue methods used by AnalOSUserDriver.
...
+@end
+
+@implementation AnalOSUserDriver
+
...
+@property(nonatomic, strong) AnalOSUserDriver* userDriver;
...
+  _userDriver = [[AnalOSUserDriver alloc] init];
...
+
+#if !defined(OFFICIAL_BUILD)
+  if (cmd->HasSwitch(analos::kSparkleDryRun)) {
+    LOG(WARNING) << "Sparkle: DRY-RUN MODE enabled";
+    _dryRunMode = YES;
+  }
+
+  if (cmd->HasSwitch(analos::kSparkleSpoofVersion)) {
+    std::string version = cmd->GetSwitchValueASCII(analos::kSparkleSpoofVersion);
+    LOG(WARNING) << "Sparkle: Spoofing version as " << version;
+    _spoofedVersion = base::SysUTF8ToNSString(version);
+  }
+
+  if (cmd->HasSwitch(analos::kSparkleVerbose)) {
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSwitch(analos::kSparkleForceCheck)) {
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSwitch(analos::kSparkleUrl)) {
+    std::string url = cmd->GetSwitchValueASCII(analos::kSparkleUrl);
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSwitch(analos::kSparkleSkipSignature)) {
