...
+    if (this.prefs && this.prefs.analos) {
+      this.loadProviders_();
+    }
+  }
+
+  private loadProviders_() {
+    // Load from preferences or initialize with AnalOS
+    if (!this.prefs || !this.prefs.analos || !this.prefs.analos.providers) {
+      // Prefs not ready yet, initialize defaults locally only
+      this.initializeDefaultProvidersLocally_();
+      return;
+    }
+    
+    const stored = this.getPref('analos.providers');
...
+        
+        // Ensure AnalOS is always present
+        if (!this.providers_.some(p => p.id === 'analos')) {
...
+    
+    const analos: ProviderConfig = {
+      id: 'analos',
+      name: 'AnalOS',
+      type: ProviderType.ANALOS,
+      isDefault: true,
+      isBuiltIn: true,
+      createdAt: now,
+      updatedAt: now,
+    };
+    
+    this.providers_ = [analos];
+    this.defaultProviderId_ = 'analos';
...
+    if (this.prefs && this.prefs.analos && this.prefs.analos.providers !== undefined) {
+      this.saveProviders_();
+    }
+  }
+
...
+    if (!this.prefs || !this.prefs.analos || this.prefs.analos.providers === undefined) {
+      console.warn('analos: Cannot save providers - prefs not ready');
+      return;
+    }
+    
...
+    this.setPrefValue('analos.providers', JSON.stringify(data));
+    console.log('analos: Saving providers:', data);
+  }
+
...
+    console.log('analos: Add Provider clicked');
+    
...
+    }
+    
+    console.log('analos: Form visibility:', this.showProviderForm_);
+  }
+
...
+    const provider = this.providers_.find(p => p.id === providerId);
+    
+    console.log('analos: Delete provider clicked:', providerId, provider);
+    
...
+      
+      // If deleted provider was default, set AnalOS as default
+      if (provider.id === this.defaultProviderId_) {
+        this.set('defaultProviderId_', 'analos');
...
+    }
+    
+    console.log('analos: Provider type changed to:', providerType);
+    
+    const defaults = PROVIDER_DEFAULTS[providerType];
+    if (defaults) {
+      console.log('analos: Applying defaults:', defaults);
...
+            console.log('analos: Auto-set context length to:', contextLength);
+          }
+          
+          this.set('showModelDropdown_', false);
+          this.set('selectedSuggestionIndex_', -1);
...
+    console.log('analos: Model suggestion clicked:', model);
+    
+    if (model) {
+      console.log('analos: Setting dialogModelId_ to:', model);
...
+      if (input) {
+        input.value = model;
+        console.log('analos: Input value set to:', input.value);
+      }
+    }
+  }
+  
...
+    console.log('analos: Custom model selected:', this.dialogModelId_);
+    
...
+      
+      chrome.send('logAnalOSMetric', ['settings.provider.added', {
...
+    console.log('analos: Validating form:', {
+      name: this.dialogProviderName_,
+      type: this.dialogProviderType_,
+      apiKey: this.dialogApiKey_,
+      modelId: this.dialogModelId_
...
+
+    chrome.send('logAnalOSMetric', ['settings.default_provider.changed', {
...
+      classes += ' selected';
+    }
+    if (provider.type === ProviderType.ANALOS) {
+      classes += ' analos';
+    }
+    return classes;
+  }
...
+    }, 600);
+    
+    console.log('analos: Using template:', template.name);
+  }
+
