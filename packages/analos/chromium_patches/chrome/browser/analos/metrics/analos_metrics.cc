diff --git a/chrome/browser/analos/metrics/analos_metrics.cc b/chrome/browser/analos/metrics/analos_metrics.cc
new file mode 100644
index 0000000000000..cf1a9a0c1790b
--- /dev/null
+++ b/chrome/browser/analos/metrics/analos_metrics.cc
@@ -0,0 +1,100 @@
...
+AnalOSMetricsService* GetMetricsService() {
...
+  return AnalOSMetricsServiceFactory::GetForBrowserContext(profile);
+}
+
...
+  } else {
+    VLOG(1) << "analos: Metrics service not available for event: " << event_name;
+  }
+}
+
+}  // namespace
+
+// static
+void AnalOSMetrics::Log(const std::string& event_name, double sample_rate) {
+  Log(event_name, base::Value::Dict(), sample_rate);
+}
+
+// static
+void AnalOSMetrics::Log(const std::string& event_name,
...
+
+// static
+void AnalOSMetrics::Log(const std::string& event_name, base::Value::Dict properties,
+                           double sample_rate) {
