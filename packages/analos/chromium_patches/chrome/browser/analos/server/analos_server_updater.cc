diff --git a/chrome/browser/analos/server/analos_server_updater.cc b/chrome/browser/analos/server/analos_server_updater.cc
new file mode 100644
index 0000000000000..62162c8924fce
--- /dev/null
+++ b/chrome/browser/analos/server/analos_server_updater.cc
@@ -0,0 +1,1061 @@
...
+#include "chrome/browser/analos/server/analos_server_constants.h"
...
+  return net::DefineNetworkTrafficAnnotation("analos_server_appcast", R"(
+    semantics {
+      sender: "AnalOS Server Updater"
+      description:
+        "Checks for updates to the AnalOS server component by fetching "
+        "an appcast XML feed."
...
+    policy {
+      cookies_allowed: NO
+      setting: "This feature can be disabled via --disable-analos-server or --disable-analos-server-updater."
+      policy_exception_justification:
+        "Essential for keeping AnalOS server component up to date."
+    })");
+}
+
+net::NetworkTrafficAnnotationTag GetDownloadTrafficAnnotation() {
+  return net::DefineNetworkTrafficAnnotation("analos_server_download", R"(
+    semantics {
+      sender: "AnalOS Server Updater"
+      description:
+        "Downloads a new version of the AnalOS server component."
...
+  return net::DefineNetworkTrafficAnnotation("analos_server_status", R"(
+    semantics {
+      sender: "AnalOS Server Updater"
+      description:
+        "Checks if the local AnalOS server is ready for hot-swap update."
...
+      setting: "This feature can be disabled via --disable-analos-server or --disable-analos-server-updater."
+      policy_exception_justification:
+        "Essential for coordinating AnalOS server updates."
+    })");
+}
+
...
+    LOG(ERROR) << "analos: Failed to decode public key from base64";
+    return false;
+  }
+  if (public_key_bytes.size() != ED25519_PUBLIC_KEY_LEN) {
+    LOG(ERROR) << "analos: Invalid public key length: "
...
+    LOG(ERROR) << "analos: Failed to decode signature from base64";
+    return false;
+  }
+  if (signature_bytes.size() != ED25519_SIGNATURE_LEN) {
+    LOG(ERROR) << "analos: Invalid signature length: "
...
+    LOG(ERROR) << "analos: Failed to read file for signature verification: "
+               << file_path;
+    return false;
+  }
+
...
+  if (result != 1) {
+    LOG(ERROR) << "analos: Ed25519 signature verification failed";
+    return false;
+  }
+
+  LOG(INFO) << "analos: Ed25519 signature verified successfully";
+  return true;
+}
+
...
+  }
+
+  LOG(INFO) << "analos: Extracted ZIP to " << dest_dir;
+  return "";  // Success
+}
+
...
+    LOG(WARNING) << "analos: Cleaning stale version directory: " << dest_dir;
...
+}  // namespace
+
+AnalOSServerUpdater::AnalOSServerUpdater(
+    analos::AnalOSServerManager* manager)
+    : manager_(manager) {}
+
+AnalOSServerUpdater::~AnalOSServerUpdater() {
+  Stop();
+}
+
+void AnalOSServerUpdater::Start() {
+  LOG(INFO) << "analos: Starting server updater";
+
...
+                            &AnalOSServerUpdater::OnUpdateTimer);
+}
+
+void AnalOSServerUpdater::LoadVersionCachesAsync() {
...
+          },
+          version_file),
+      base::BindOnce(&AnalOSServerUpdater::OnDownloadedVersionLoaded,
...
+          bundled_binary),
+      base::BindOnce(
+          [](base::WeakPtr<AnalOSServerUpdater> self,
...
+}
+
+void AnalOSServerUpdater::OnDownloadedVersionLoaded(
+    const std::string& version_str) {
+  if (!version_str.empty()) {
+    cached_downloaded_version_ = base::Version(version_str);
+    LOG(INFO) << "analos: Cached downloaded version: "
...
+}
+
+void AnalOSServerUpdater::OnBundledVersionLoaded(
+    int exit_code,
...
+      LOG(INFO) << "analos: Cached bundled version: "
+                << cached_bundled_version_.GetString();
+    } else {
+      LOG(WARNING) << "analos: Could not parse bundled version from: "
+                   << output;
+    }
+  } else {
+    LOG(WARNING) << "analos: Failed to get bundled version (exit_code="
+                 << exit_code << ")";
+  }
...
+}
+
+void AnalOSServerUpdater::CheckVersionCachesAndStart() {
...
+}
+
+void AnalOSServerUpdater::Stop() {
+  LOG(INFO) << "analos: Stopping server updater";
...
+}
+
+void AnalOSServerUpdater::CheckNow() {
+  if (!bundled_version_loaded_ || !downloaded_version_loaded_) {
+    LOG(INFO) << "analos: Version caches not loaded yet, skipping check";
+    return;
+  }
+
+  if (update_in_progress_) {
+    LOG(INFO) << "analos: Update check already in progress, skipping";
+    return;
+  }
+
+  FetchAppcast();
+}
+
+void AnalOSServerUpdater::OnUpdateTimer() {
+  CheckNow();
+}
+
+void AnalOSServerUpdater::FetchAppcast() {
...
+  if (cmd->HasSwitch(analos::kServerAppcastUrl)) {
+    appcast_url = cmd->GetSwitchValueASCII(analos::kServerAppcastUrl);
+    LOG(INFO) << "analos: Using custom appcast URL: " << appcast_url;
+  } else if (base::FeatureList::IsEnabled(features::kAnalOsAlphaFeatures)) {
...
+  }
+
+  LOG(INFO) << "analos: Fetching appcast from " << url;
+
...
+      url_loader_factory,
+      base::BindOnce(&AnalOSServerUpdater::OnAppcastFetched,
+                     weak_factory_.GetWeakPtr()),
+      kMaxAppcastSize);
+}
+
+void AnalOSServerUpdater::OnAppcastFetched(
...
+  }
+
+  LOG(INFO) << "analos: Received appcast (" << response->size() << " bytes)";
+
+  // Parse the appcast
+  std::optional<AppcastItem> item =
+      AnalOSAppcastParser::ParseLatestItem(*response);
+  if (!item) {
+    OnError("check", "Failed to parse appcast XML");
+    return;
+  }
+
+  LOG(INFO) << "analos: Latest version in appcast: "
...
+  }
+
+  LOG(INFO) << "analos: Found enclosure for current platform: "
+            << enclosure->url;
+
+  // Compare with current version
+  base::Version current = GetCurrentVersion();
+  LOG(INFO) << "analos: Current version: "
+            << (current.IsValid() ? current.GetString() : "(none)");
+
+  if (current.IsValid() && current >= item->version) {
+    LOG(INFO) << "analos: Already up to date";
+    ResetState();
+    return;
+  }
+
+  LOG(INFO) << "analos: New version available: " << item->version.GetString();
+  pending_item_ = *item;
+  pending_signature_ = enclosure->signature;
+  CheckVersionAlreadyDownloaded(*enclosure, item->version);
+}
+
+void AnalOSServerUpdater::CheckVersionAlreadyDownloaded(
...
+      base::BindOnce(&AnalOSServerUpdater::OnVersionExistsCheck,
+                     weak_factory_.GetWeakPtr(), enclosure, version));
+}
+
+void AnalOSServerUpdater::OnVersionExistsCheck(
+    const AppcastEnclosure& enclosure,
+    const base::Version& version,
+    bool exists) {
+  if (exists) {
+    LOG(INFO) << "analos: Version " << version.GetString()
...
+}
+
+void AnalOSServerUpdater::StartDownload(const AppcastEnclosure& enclosure,
+                                           const base::Version& version) {
...
+            LOG(INFO) << "analos: Downloading " << download_url;
+
+            auto request = std::make_unique<network::ResourceRequest>();
...
+                url_loader_factory,
+                base::BindOnce(&AnalOSServerUpdater::OnDownloadComplete,
+                               self, ver),
+                download_path);
+          },
+          weak_factory_.GetWeakPtr(), enclosure, version));
+}
+
+void AnalOSServerUpdater::OnDownloadComplete(const base::Version& version,
...
+  }
+
+  LOG(INFO) << "analos: Download complete: " << zip_path;
+
+  // Now verify and extract
+  VerifyAndExtract(zip_path, pending_signature_, version);
+}
+
+void AnalOSServerUpdater::VerifyAndExtract(const base::FilePath& zip_path,
+                                              const std::string& signature,
+                                              const base::Version& version) {
...
+
+  LOG(INFO) << "analos: Verifying signature and extracting to " << dest_dir;
+
...
+}
+
+void AnalOSServerUpdater::OnVerifyAndExtractComplete(
+    const base::Version& version,
+    bool success,
...
+  }
+
+  LOG(INFO) << "analos: Verification and extraction successful";
+
+  // Test the binary
+  TestBinary(version);
+}
+
+void AnalOSServerUpdater::TestBinary(const base::Version& version) {
+  state_ = State::kTesting;
+
+  base::FilePath binary_path = GetDownloadedBinaryPath(version);
+  LOG(INFO) << "analos: Testing binary: " << binary_path;
+
...
+          binary_path),
+      base::BindOnce(
+          [](base::WeakPtr<AnalOSServerUpdater> self, base::Version version,
+             std::pair<int, std::string> result) {
+            if (!self)
+              return;
...
+}
+
+void AnalOSServerUpdater::OnBinaryTestComplete(const base::Version& version,
+                                                  int exit_code,
+                                                  const std::string& output) {
+  if (exit_code != 0) {
+    LOG(ERROR) << "analos: Binary test failed with exit code " << exit_code
+               << ": " << output;
+
...
+  }
+
+  LOG(INFO) << "analos: Binary test passed: " << output;
+
+  // Check if server is ready for hot-swap
+  CheckServerStatus();
+}
+
+void AnalOSServerUpdater::CheckServerStatus() {
+  GURL status_url("http://127.0.0.1:" +
+                  base::NumberToString(manager_->GetMCPPort()) + "/status");
+
+  LOG(INFO) << "analos: Checking server status at " << status_url;
+
+  auto request = std::make_unique<network::ResourceRequest>();
...
+      url_loader_factory,
+      base::BindOnce(&AnalOSServerUpdater::OnStatusFetched,
+                     weak_factory_.GetWeakPtr()),
+      4096);
+}
+
+void AnalOSServerUpdater::OnStatusFetched(
+    std::unique_ptr<std::string> response) {
+  if (!response) {
+    int net_error = status_loader_->NetError();
+    LOG(WARNING) << "analos: Failed to fetch server status: "
...
+    LOG(WARNING) << "analos: Invalid status response, proceeding with update";
+    OnServerStatusChecked(/*can_update=*/true);
+    return;
+  }
+
...
+    LOG(WARNING) << "analos: Status response missing can_update field";
+    OnServerStatusChecked(/*can_update=*/true);
+    return;
+  }
+
...
+}
+
+void AnalOSServerUpdater::OnServerStatusChecked(bool can_update) {
+  if (!can_update) {
+    LOG(INFO) << "analos: Server busy, will retry hot-swap at next check";
+
+    base::Value::Dict props;
+    props.Set("pending_version", pending_item_.version.GetString());
+    analos_metrics::AnalOSMetrics::Log("server.ota.busy", std::move(props));
+
+    ResetState();
+    return;
+  }
...
+}
+
+void AnalOSServerUpdater::PerformHotSwap(const base::Version& version) {
+  LOG(INFO) << "analos: Performing hot-swap to version "
+            << version.GetString();
+
...
+      base::BindOnce(&AnalOSServerUpdater::OnHotSwapComplete,
+                     weak_factory_.GetWeakPtr(), old_version, version));
+}
+
+void AnalOSServerUpdater::OnHotSwapComplete(const base::Version& old_version,
+                                               const base::Version& new_version,
+                                               bool success) {
+  if (!success) {
+    LOG(ERROR) << "analos: Hot-swap failed, server may need manual restart";
+    OnError("hotswap", "Failed to restart server with new binary");
+    return;
+  }
+
+  LOG(INFO) << "analos: Hot-swap successful! Now running version "
...
+  analos_metrics::AnalOSMetrics::Log("server.ota.success",
+                                           std::move(props));
+
+  ResetState();
+}
+
+base::Version AnalOSServerUpdater::GetCurrentVersion() {
...
+}
+
+void AnalOSServerUpdater::InvalidateDownloadedVersion() {
+  LOG(WARNING) << "analos: Invalidating downloaded version, "
...
+                LOG(ERROR) << "analos: Failed to delete versions directory: "
+                           << versions_dir;
+              }
+            }
+            if (base::PathExists(version_file)) {
+              if (!base::DeleteFile(version_file)) {
+                LOG(ERROR) << "analos: Failed to delete current_version file: "
+                           << version_file;
+              }
+            }
+          },
...
+}
+
+base::FilePath AnalOSServerUpdater::GetExecutionDir() const {
+  base::FilePath user_data_dir;
+  if (!base::PathService::Get(chrome::DIR_USER_DATA, &user_data_dir)) {
+    return base::FilePath();
+  }
+  return user_data_dir.Append(FILE_PATH_LITERAL(".analos"));
+}
+
+base::FilePath AnalOSServerUpdater::GetVersionsDir() const {
+  return GetExecutionDir().AppendASCII(kVersionsDirectoryName);
+}
+
+base::FilePath AnalOSServerUpdater::GetVersionDir(
+    const base::Version& version) const {
+  return GetVersionsDir().AppendASCII(version.GetString());
+}
+
+base::FilePath AnalOSServerUpdater::GetPendingUpdateDir() const {
+  return GetExecutionDir().AppendASCII(kPendingUpdateDirectoryName);
+}
+
+base::FilePath AnalOSServerUpdater::GetBundledBinaryPath() const {
+  // Delegate to manager's existing logic
+  return manager_->GetAnalOSServerExecutablePath();
+}
+
+base::FilePath AnalOSServerUpdater::GetBundledResourcesPath() const {
+  return manager_->GetAnalOSServerResourcesPath();
+}
+
+base::FilePath AnalOSServerUpdater::GetDownloadedBinaryPath(
+    const base::Version& version) const {
...
+}
+
+base::FilePath AnalOSServerUpdater::GetDownloadedResourcesPath(
+    const base::Version& version) const {
+  return GetVersionDir(version).Append(FILE_PATH_LITERAL("resources"));
+}
+
+base::FilePath AnalOSServerUpdater::GetBestServerBinaryPath() {
...
+}
+
+base::FilePath AnalOSServerUpdater::GetBestServerResourcesPath() {
+  // Use cached versions to avoid blocking I/O
+  base::Version downloaded = cached_downloaded_version_;
+  base::Version bundled = cached_bundled_version_;
+
...
+}
+
+void AnalOSServerUpdater::CleanupPendingUpdate() {
+  base::FilePath pending_dir = GetPendingUpdateDir();
+  base::ThreadPool::PostTask(
+      FROM_HERE, {base::MayBlock()},
+      base::BindOnce(
...
+}
+
+void AnalOSServerUpdater::CleanupOldVersions() {
+  base::FilePath versions_dir = GetVersionsDir();
+
+  base::ThreadPool::PostTask(
+      FROM_HERE, {base::MayBlock()},
...
+              LOG(INFO) << "analos: Cleaning up old version: "
...
+              analos_metrics::AnalOSMetrics::Log("server.ota.cleanup",
+                                                       std::move(props));
+            }
+          },
...
+}
+
+void AnalOSServerUpdater::OnError(const std::string& stage,
+                                     const std::string& error) {
+  LOG(ERROR) << "analos: Update error at " << stage << ": " << error;
+
+  base::Value::Dict props;
...
+  }
+  analos_metrics::AnalOSMetrics::Log("server.ota.error", std::move(props));
+
...
+            [](base::FilePath dir) {
+              if (base::PathExists(dir)) {
+                LOG(INFO) << "analos: Cleaning up failed version: " << dir;
+                base::DeletePathRecursively(dir);
+              }
+            },
+            version_dir));
...
+}
+
+void AnalOSServerUpdater::ResetState() {
+  state_ = State::kIdle;
