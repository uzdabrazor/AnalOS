diff --git a/chrome/browser/analos/server/validate_resources.py b/chrome/browser/analos/server/validate_resources.py
new file mode 100644
index 0000000000000..d7dc82b132dad
--- /dev/null
+++ b/chrome/browser/analos/server/validate_resources.py
@@ -0,0 +1,43 @@
+#!/usr/bin/env python3
...
+
+"""Validates that required AnalOS resources exist.
+
...
+    print(f"ERROR: Required AnalOS resource not found: {resource_path}")
+    all_valid = False
+    continue
+
...
+  sys.exit(1)
+
+print(f"✓ AnalOS resources validated ({len(REQUIRED_RESOURCES)} resources)")
+sys.exit(0)
