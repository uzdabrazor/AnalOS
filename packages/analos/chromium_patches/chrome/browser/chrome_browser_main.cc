...
+      browser_creator_->AddFirstRunTabs({GURL("chrome://analos-first-run")});
+      browser_creator_->AddFirstRunTabs({GURL("https://bit.ly/AnalOS-setup")});
...
+  // AnalOS: Start the AnalOS server after browser initialization
+  LOG(INFO) << "analos: Starting AnalOS server process";
+  analos::AnalOSServerManager::GetInstance()->Start();
+
 #if BUILDFLAG(IS_WIN)
...
+
+  // AnalOS: Stop the AnalOS server during shutdown
+  LOG(INFO) << "analos: Stopping AnalOS server process";
+  analos::AnalOSServerManager::GetInstance()->Shutdown();
+
