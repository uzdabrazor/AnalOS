...
+#include "chrome/browser/analos/metrics/analos_metrics_prefs.h"
...
+  analos_server::RegisterLocalStatePrefs(registry);
+  analos_metrics::RegisterLocalStatePrefs(registry);
...
+  analos_metrics::RegisterProfilePrefs(registry);
...
+void RegisterAnalOSPrefs(user_prefs::PrefRegistrySyncable* registry) {
...
+  registry->RegisterStringPref(prefs::kAnalOSProviders, "");
+  
+  // AnalOS toolbar settings
+  registry->RegisterBooleanPref(prefs::kAnalOSShowToolbarLabels, true);
+  
+  // Custom providers list - stored as a JSON string
+  registry->RegisterStringPref(prefs::kAnalOSCustomProviders, "[]");
+}
+
 #if BUILDFLAG(IS_CHROMEOS)
