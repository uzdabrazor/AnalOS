...
+// Handles chrome://analos/* URLs by rewriting to extension URLs.
+// Forward handler: chrome://analos/ai -> chrome-extension://[id]/options.html
+static bool HandleAnalOSURL(GURL* url,
+                               content::BrowserContext* browser_context) {
+  if (!url->SchemeIs(content::kChromeUIScheme) ||
+      url->host() != analos::kAnalOSHost) {
+    return false;
+  }
+
+  std::string extension_url =
+      analos::GetAnalOSExtensionURL(url->path());
...
+}
+
+// Reverse handler: chrome-extension://[id]/options.html#ai -> chrome://analos/ai
+// This ensures the virtual URL is shown in the address bar.
+static bool ReverseAnalOSURL(GURL* url,
+                                content::BrowserContext* browser_context) {
...
+  // Handler to rewrite chrome://analos/* to extension URLs.
+  handler->AddHandlerPair(&HandleAnalOSURL, &ReverseAnalOSURL);
...
+#if BUILDFLAG(ENABLE_EXTENSIONS_CORE)
+  // Allow AnalOS extensions to access private networks (e.g., localhost).
+  // This enables extension service workers to connect to local servers.
+  if (origin.scheme() == extensions::kExtensionScheme &&
+      analos::IsAnalOSExtension(origin.host())) {
