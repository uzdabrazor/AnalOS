...
+  }
+
+  // Pin native AnalOS actions if their features are enabled (or no feature flag)
+  for (actions::ActionId id : analos::kAnalOSNativeActionIds) {
+    const base::Feature* feature = analos::GetFeatureForAnalOSAction(id);
