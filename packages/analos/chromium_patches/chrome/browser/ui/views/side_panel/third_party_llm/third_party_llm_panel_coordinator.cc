...
+
+// Preference names
+const char kThirdPartyLlmProvidersPref[] = "analos.third_party_llm.providers";
+const char kThirdPartyLlmSelectedProviderPref[] = "analos.third_party_llm.selected_provider";
+
...
+    LOG(INFO) << "[analos] Provider list size changed from " << previous_size
...
+  
+  analos_metrics::AnalOSMetrics::Log("llmchat.created");
+  
+  return container;
+}
+
...
+  if (!prefs) {
+    LOG(ERROR) << "[analos] Failed to get PrefService";
...
+      if (!item.is_dict()) {
+        LOG(WARNING) << "[analos] Invalid provider entry (not a dict), skipping";
+        continue;
+      }
+
...
+
+      if (!name || name->empty()) {
+        LOG(WARNING) << "[analos] Provider missing name, skipping";
+        continue;
+      }
+
+      if (!url || url->empty()) {
+        LOG(WARNING) << "[analos] Provider missing URL, skipping";
+        continue;
+      }
+
+      GURL provider_url(*url);
+      if (!provider_url.is_valid()) {
+        LOG(WARNING) << "[analos] Invalid provider URL: " << *url;
+        continue;
+      }
+
...
+  if (providers_.empty()) {
+    LOG(INFO) << "[analos] No providers in prefs, using defaults";
+    providers_ = GetDefaultProviders();
...
+    LOG(WARNING) << "[analos] Invalid selected provider index: " << selected_index
+                 << ", resetting to 0";
+    current_provider_index_ = 0;
+    prefs->SetInteger(kThirdPartyLlmSelectedProviderPref, 0);
+  } else {
...
+  PrefService* prefs = GetProfile()->GetPrefs();
+  if (!prefs) {
+    LOG(ERROR) << "[analos] Failed to get PrefService for saving";
+    return;
+  }
+
...
+    LOG(ERROR) << "[analos] Invalid provider index: " << new_provider_index;
+    return;
+  }
+
+  provider_change_in_progress_ = true;
+
+  analos_metrics::AnalOSMetrics::Log("llmchat.provider.changed");
+
+  if (owned_web_contents_) {
+    GURL current_url = owned_web_contents_->GetURL();
+    if (current_url.is_valid()) {
...
+  
+  analos_metrics::AnalOSMetrics::Log("llmchat.screenshot.captured");
+
...
+    
+    analos_metrics::AnalOSMetrics::Log("llmchat.content.copied");
+    
...
+  LOG(INFO) << "[analos] Media access requested for origin: "
...
+
+    LOG(INFO) << "[analos] Auto-granting media permission for trusted LLM: "
+              << origin_url.host();
+
...
+      VLOG(2) << "[analos] Media permission check for trusted LLM origin: "
...
+
+  VLOG(2) << "[analos] Media permission check for origin: "
+          << security_origin.Serialize()
+          << " type: " << static_cast<int>(type)
