...
+
+// Preference names
+const char kClashOfGptsPaneProvidersPref[] = "analos.clash_of_gpts.pane_providers";  // Per-pane selections
+const char kClashOfGptsLastUrlsPref[] = "analos.clash_of_gpts.last_urls";
+const char kClashOfGptsPaneCountPref[] = "analos.clash_of_gpts.pane_count";
+
...
+    widget_->Show();
+    widget_->Activate();
+    analos_metrics::AnalOSMetrics::Log("llmhub.shown");
+  }
+}
+
...
+    LOG(ERROR) << "[analos] Invalid provider index: " << provider_index;
+    return;
+  }
+
...
+  SaveState();
+  
+  analos_metrics::AnalOSMetrics::Log("llmhub.panecount.changed", 
