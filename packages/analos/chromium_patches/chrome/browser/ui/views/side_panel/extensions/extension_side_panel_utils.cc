...
+  LOG(INFO) << "analos: IsContextualExtensionSidePanelOpen called for extension="
+            << extension_id;
+
+  if (!browser_window || !web_contents) {
+    LOG(WARNING) << "analos: browser_window or web_contents is null";
+    return false;
+  }
+
...
+  LOG(INFO) << "analos: is_active_tab=" << is_active_tab;
+
...
+    LOG(INFO) << "analos: side_panel is_showing=" << is_showing;
+    if (!is_showing) {
+      return false;
+    }
...
+    LOG(INFO) << "analos: contextual panel is_active=" << is_active;
+    return is_active;
+  }
+
...
+      tab->GetTabFeatures()->side_panel_registry();
+  bool is_active = IsKeyActiveInRegistry(contextual_registry, extension_key);
+  LOG(INFO) << "analos: inactive tab contextual panel is_active=" << is_active;
+  return is_active;
+}
+
...
+  LOG(INFO) << "analos: ToggleContextualExtensionSidePanel called for extension="
...
+  }
+
+  LOG(INFO) << "analos: is_currently_open=" << is_currently_open
...
+
+    if (is_active_tab) {
+      LOG(INFO) << "analos: Calling side_panel_ui->Show() for active tab";
