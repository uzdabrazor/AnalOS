...
class ArtifactRegistry:
    ...
    Example:
        ...
        artifacts.add("built_app", Path("/path/to/AnalOS.app"))
...
class BuildConfig:
    ...
    def __init__(
        ...
        self.analos_version = ""
        self.analos_chromium_version = ""

        # App names - will be set based on platform
        self.CHROMIUM_APP_NAME = ""
        self.ANALOS_APP_NAME = ""
        self.ANALOS_APP_BASE_NAME = "AnalOS"
    ...
    def _set_app_names(self):
        ...
        if IS_WINDOWS():
            ...
            self.ANALOS_APP_NAME = (
                f"{self.ANALOS_APP_BASE_NAME}{get_executable_extension()}"
            )
        elif IS_MACOS():
            self.CHROMIUM_APP_NAME = "Chromium.app"
            self.ANALOS_APP_NAME = f"{self.ANALOS_APP_BASE_NAME}.app"
        else:
            self.CHROMIUM_APP_NAME = "chrome"
            self.ANALOS_APP_NAME = self.ANALOS_APP_BASE_NAME.lower()
...
class Context:
    ...
    analos_build_offset: str = ""
    analos_chromium_version: str = ""
    semantic_version: str = ""  # e.g., "0.31.0" from resources/ANALOS_VERSION
    ...
    ANALOS_APP_NAME: str = ""
    ANALOS_APP_BASE_NAME: str = "AnalOS"  # Base name without extension
    ...
    def __post_init__(self):
        ...
        self.build.ANALOS_APP_NAME = self.ANALOS_APP_NAME
        ...
        if not self.analos_build_offset:
            self.analos_build_offset = self._load_analos_build_offset(
                self.root_dir
            )

        # Load semantic version from resources/ANALOS_VERSION
        ...
        if self.chromium_version and self.analos_build_offset and version_dict:
            # Calculate new BUILD number by adding nxtscape_version to original BUILD
            new_build = int(version_dict["BUILD"]) + int(self.analos_build_offset)
            self.analos_chromium_version = f"{version_dict['MAJOR']}.{version_dict['MINOR']}.{new_build}.{version_dict['PATCH']}"

        # Sync versions with BuildConfig
        self.build.chromium_version = self.chromium_version
        self.build.analos_version = self.analos_build_offset
        self.build.analos_chromium_version = self.analos_chromium_version
    ...
    @staticmethod
    def _load_analos_build_offset(root_dir: Path) -> str:
        """Load analos build offset from config/ANALOS_BUILD_OFFSET"""
        version_file = join_paths(root_dir, "build", "config", "ANALOS_BUILD_OFFSET")
        if version_file.exists():
    ...
    def _load_semantic_version(root_dir: Path) -> str:
        ...
        """
        version_file = join_paths(root_dir, "resources", "ANALOS_VERSION")
        ...
        major = version_dict.get("ANALOS_MAJOR", "0")
        minor = version_dict.get("ANALOS_MINOR", "0")
        build = version_dict.get("ANALOS_BUILD", "0")
        patch = version_dict.get("ANALOS_PATCH", "0")
    ...
    def get_app_path(self) -> Path:
        ...
        For universal builds, checks if out/Default_universal/AnalOS.app exists
        ...
        if IS_MACOS():
            universal_app = join_paths(
                self.chromium_src, "out/Default_universal", self.ANALOS_APP_NAME
            )
        ...
        if self.build_type == "debug" and IS_MACOS():
            ...
            debug_app_name = f"{self.ANALOS_APP_BASE_NAME} Dev.app"
        ...
        return join_paths(self.chromium_src, self.out_dir, self.ANALOS_APP_NAME)
    ...
    def get_analos_chromium_version(self) -> str:
        """Get analos chromium version string"""
        return self.analos_chromium_version

    def get_analos_version(self) -> str:
        """Get analos version string (build offset)"""
        return self.analos_build_offset

    def get_semantic_version(self) -> str:
        """Get semantic version from resources/ANALOS_VERSION
    ...
    def get_sparkle_version(self) -> str:
        """Get Sparkle-compatible version from analos_chromium_version

        Sparkle uses BUILD.PATCH format for version comparison.
        Returns: e.g., "7231.69"
        """
        if not self.analos_chromium_version:
            raise ValueError("analos_chromium_version is not set")

        parts = self.analos_chromium_version.split(".")
        if len(parts) < 4:
            raise ValueError(
                f"Invalid analos_chromium_version format: {self.analos_chromium_version}"
            )
    ...
    def get_app_base_name(self) -> str:
        ...
        return self.ANALOS_APP_BASE_NAME
