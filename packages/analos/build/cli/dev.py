...
def main(
    ...
    Extract patches from commits:
      analos dev extract commit HEAD
      analos dev extract range HEAD~5 HEAD

    Apply patches:
      analos dev apply all
      analos dev apply feature llm-chat

    Manage features:
      analos dev feature list
      analos dev feature add my-feature HEAD
      analos dev feature show my-feature
    """
...
def apply_changed(
    ...
    Examples:
        ...
        analos dev apply changed --commit 1c78477 --reset-to base -S /chromium

        # Apply patches changed in a range of commits
        analos dev apply changed --range-start HEAD~5 --range-end HEAD --reset-to base -S /chromium
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
...
def feature_add_update(
    ...
    Examples:
        analos dev feature add-update --name llm-chat --commit HEAD -d "feat: LLM chat panel"
        analos dev feature add-update -n api -c HEAD~3 -d "feat: analos API updates"
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
...
def annotate_cmd(
    ...
    Examples:
        analos dev annotate -S /path/to/chromium
        analos dev annotate llm-chat -S /path/to/chromium
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
