#!/usr/bin/env python3
"""Build CLI - Modular build system for AnalOS"""
...
def main(
    ...
    """AnalOS Build System - Modular pipeline executor

    Build AnalOS using phase flags (auto-ordered), explicit modules, or configs.

    \b
    Phase Flags (Recommended - Auto-Ordered):
      analos build --setup --build --sign --package
      analos build --build --sign           # Skip setup
      analos build --package --sign         # Flags work in any order!

    \b
    Explicit Modules (Power Users):
      analos build --modules clean,compile,sign_macos

    \b
    Config Files (CI/CD):
      analos build --config release.yaml --arch arm64

    \b
    List Available:
      analos build --list                   # Show all modules and phases
    ...
    if options_provided > 1:
        ...
        log_error("Examples:")
        log_error("  analos build --setup --build --sign")
        log_error("  analos build --modules clean,compile")
        log_error("  analos build --config release.yaml")
        raise typer.Exit(1)
    ...
    if has_flags:
        ...
        if prep:
            ...
            log_warning("⚠️  --prep does NOT apply series_patches. Run 'analos build -m series_patches' separately if needed.")
        if build:
    ...
    log_info(f"📍 Build offset: {ctx.analos_build_offset}")
