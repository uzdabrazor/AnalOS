...
Design:
    ...
    Output: 3 DMGs uploaded:
        - AnalOS_{version}_arm64_signed.dmg
        - AnalOS_{version}_x64_signed.dmg
        - AnalOS_{version}_universal_signed.dmg
...
class UniversalBuildModule(CommandModule):
    ...
    def execute(self, ctx: Context) -> None:
        ...
        for arch in UNIVERSAL_ARCHITECTURES:
            ...
            log_info(f"📍 AnalOS: {arch_ctx.analos_build_offset}")
            log_info(f"📍 Output directory: {arch_ctx.out_dir}")

            # === BUILD PHASE ===
            # Copy resources (arch-specific binaries like analos_server, codex)
        ...
        universal_app = ctx.chromium_src / "out/Default_universal/AnalOS.app"
    ...
    def _create_arch_context(self, base_ctx: Context, arch: str) -> Context:
        ...
        ctx._fixed_app_path = (
            ctx.chromium_src / f"out/Default_{arch}" / ctx.ANALOS_APP_NAME
        )
        return ctx
    ...
    def _create_universal_context(self, base_ctx: Context) -> Context:
        ...
        ctx._fixed_app_path = (
            ctx.chromium_src / "out/Default_universal" / ctx.ANALOS_APP_NAME
        )
    ...
    def _merge_universal(
        ...
        universal_app = universal_dir / "AnalOS.app"
