#!/usr/bin/env python3
"""Linux packaging module for AnalOS (AppImage and .deb)"""
...
class LinuxPackageModule(CommandModule):
    ...
    def validate(self, ctx: Context) -> None:
        ...
        chrome_binary = join_paths(out_dir, ctx.ANALOS_APP_NAME)
    ...
    def execute(self, ctx: Context) -> None:
        log_info(
            f"\n📦 Packaging {ctx.ANALOS_APP_BASE_NAME} {ctx.get_analos_chromium_version()} for Linux ({ctx.architecture})"
        )
...
def copy_browser_files(
    ...
    dirs_to_copy = ["locales", "MEIPreload", "AnalOSServer"]
    ...
    analos_path = Path(join_paths(target_dir, ctx.ANALOS_APP_NAME))
    if analos_path.exists():
        analos_path.chmod(0o755)
...
"""
    ...
    desktop_file = Path(join_paths(apps_dir, "analos.desktop"))
...
def copy_icon(ctx: Context, icons_dir: Path) -> bool:
    ...
    icon_dest = Path(join_paths(icons_dir, "256x256", "apps", "analos.png"))
...
def prepare_appdir(ctx: Context, appdir: Path) -> bool:
    ...
    app_root = join_paths(appdir, "opt", "analos")
    ...
    desktop_file = create_desktop_file(
        apps_dir, f"/opt/analos/{ctx.ANALOS_APP_NAME}"
    )
...
THIS="$(readlink -f "${{0}}")"
HERE="$(dirname "${{THIS}}")"
export LD_LIBRARY_PATH="${{HERE}}"/opt/analos:$LD_LIBRARY_PATH
export CHROME_WRAPPER="${{THIS}}"
"${{HERE}}"/opt/analos/{ctx.ANALOS_APP_NAME} "$@"
"""
...
def create_launcher_script(ctx: Context, bin_dir: Path) -> None:
    """Create launcher script in /usr/bin/analos."""
    bin_dir.mkdir(parents=True, exist_ok=True)

    launcher_content = f"""#!/bin/sh
# AnalOS launcher script
export LD_LIBRARY_PATH=/usr/lib/analos:$LD_LIBRARY_PATH
exec /usr/lib/analos/{ctx.ANALOS_APP_NAME} "$@"
"""

    launcher_path = Path(join_paths(bin_dir, "analos"))
...
Maintainer: AnalOS Team <support@uzdabrazor.com>
Homepage: https://www.uzdabrazor.com/
Description: AnalOS - The open source agentic browser
 AnalOS is a privacy-focused web browser built on Chromium,
...
# Post-installation script for AnalOS
set -e

# Set SUID bit on chrome_sandbox for sandboxing support
if [ -f /usr/lib/analos/chrome_sandbox ]; then
    chmod 4755 /usr/lib/analos/chrome_sandbox
fi
...
def prepare_debdir(ctx: Context, debdir: Path) -> bool:
    ...
    │   │   └── [all browser files]
    │   └── share/
    │       ├── applications/analos.desktop
    │       └── icons/hicolor/256x256/apps/analos.png
    """
    log_info("📁 Preparing .deb directory structure...")

    lib_dir = join_paths(debdir, "usr", "lib", "analos")
    ...
    # Create desktop file
    create_desktop_file(apps_dir, "/usr/bin/analos")
...
def package_appimage(ctx: Context, package_dir: Path) -> Optional[Path]:
    ...
    appdir = Path(join_paths(package_dir, f"{ctx.ANALOS_APP_BASE_NAME}.AppDir"))
    if appdir.exists():
        safe_rmtree(appdir)
...
def package_deb(ctx: Context, package_dir: Path) -> Optional[Path]:
    ...
    debdir = Path(join_paths(package_dir, f"{ctx.ANALOS_APP_BASE_NAME}_deb"))
    if debdir.exists():
        safe_rmtree(debdir)
