#!/usr/bin/env python3
"""Application signing and notarization module for AnalOS (macOS)"""
...
# Central list of AnalOS Server binaries we need to sign explicitly.
...
ANALOS_SERVER_BINARIES: Dict[str, Dict[str, str]] = {
    "analos_server": {
        "identifier_suffix": "analos_server",
        "options": "runtime",
        "entitlements": "analos-executable-entitlements.plist",
    },
    "codex": {
...
def get_analos_server_binary_info(component_path: Path) -> Optional[Dict[str, str]]:
    """Return metadata for known AnalOS Server binaries, if applicable."""
    name = component_path.stem.lower()
    return ANALOS_SERVER_BINARIES.get(name)
...
class MacOSSignModule(CommandModule):
    ...
    def execute(self, ctx: Context) -> None:
        log_info("=" * 70)
        log_info("🚀 Starting signing process for AnalOS...")
        log_info("=" * 70)
...
def find_components_to_sign(
    ...
    # Check both versioned and non-versioned paths for AnalOS Framework
    # Handle both release and debug framework names
    framework_names = [
        "AnalOS Framework.framework",
        "AnalOS Dev Framework.framework",
    ]
    ...
    for fw_name in framework_names:
        ...
        if fw_path.exists():
            ...
            if ctx and ctx.analos_chromium_version:
                versioned_path = join_paths(
                    fw_path, "Versions", ctx.analos_chromium_version
                )
    ...
    # Find all dylibs (check versioned path for AnalOS Framework libraries)
    for nxtscape_fw_path in nxtscape_framework_paths:
    ...
    # Find AnalOS Server binaries
    analos_server_dir = join_paths(app_path, "Contents", "Resources", "AnalOSServer")
    if analos_server_dir.exists():
        for item in analos_server_dir.rglob("*"):
            if item.is_file() and not item.suffix and os.access(item, os.X_OK):
                components["executables"].append(item)
...
def get_identifier_for_component(
    component_path: Path, base_identifier: str = "com.analos"
) -> str:
    ...
    # AnalOS Server binaries share the same entitlements/options but need unique identifiers.
    analos_server_info = get_analos_server_binary_info(component_path)
    if analos_server_info:
        suffix = analos_server_info.get("identifier_suffix", component_path.stem)
    ...
    if component_path.suffix == ".framework":
        if name == "AnalOS Framework" or name == "AnalOS Dev Framework":
...
def get_signing_options(component_path: Path) -> str:
    ...
    # Known AnalOS Server binaries share the same relaxed options.
    analos_server_info = get_analos_server_binary_info(component_path)
    if analos_server_info:
        return analos_server_info.get("options", "runtime")
...
def sign_all_components(
    ...
    if components["executables"]:
        ...
        for exe in components["executables"]:
            ...
            entitlements = None
            analos_server_info = get_analos_server_binary_info(exe)
            if analos_server_info:
                entitlements_name = analos_server_info.get("entitlements")
    ...
    # 6. Sign frameworks (except the main AnalOS Framework)
    if components["frameworks"]:
        log_info("\n🔏 Signing frameworks...")
        # Sort to sign Sparkle.framework before AnalOS Framework.framework
    ...
    main_exe_names = ["AnalOS", "AnalOS Dev"]
    main_exe = None
    ...
    if not sign_component(main_exe, certificate_name, "com.analos.AnalOS"):
        return False

    # 8. Finally sign the app bundle
    log_info("\n🔏 Signing application bundle...")
    requirements = (
        '=designated => identifier "com.analos.AnalOS" and '
...
def sign_universal(contexts: List[Context]) -> bool:
    ...
    universal_app_path = join_paths(universal_dir, contexts[0].ANALOS_APP_NAME)
