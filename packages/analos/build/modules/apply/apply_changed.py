"""
Apply Changed - Apply patches that changed in specific commits of the analos repo.
...
class PatchChange:
    ...
    patch_path: str  # Path relative to analos repo (e.g., chromium_patches/chrome/foo.cc)
...
class ApplyChangedModule(CommandModule):
    ...
    def execute(
        ...
        # ctx.root_dir might be a subdir of git_root (e.g., packages/analos)
        ...
        # Validate commits exist in analos repo
        if commit:
            if not validate_commit_exists(commit, ctx.root_dir):
                raise RuntimeError(f"Commit not found in analos repo: {commit}")
