...
const DEFAULT_PROVIDERS: Provider[] = [
  {
    ...
    available: true,
    iconUrl: '/assets/new_tab_search/analos.svg'
  },
  {
    id: 'chatgpt',
    name: 'ChatGPT',
...
export const useProviderStore = create<ProviderState & ProviderActions>()(
  persist(
    (set, get) => ({
      ...
      customProviders: [],
      selectedProviderId: 'analos-agent',
      isDropdownOpen: false,
      enabledProviderIds: ['analos-agent', 'chatgpt', 'claude', 'google'],
      ...
      removeCustomProvider: id => {
        set(state => {
          ...
          const nextSelected = isSelected
            ? normalized.enabledProviderIds[0] || 'analos-agent'
      ...
      importLegacyProviderSettings: () => {
        ...
        try {
          ...
          const mapLegacyId = (id: string) => (id === 'analos' ? 'analos-agent' : id)
          ...
          try {
            ...
            if (needsDomWork && tabId != null) {
              ...
              while (Date.now() - start < CHAT_PROVIDER_READY_TIMEOUT_MS) {
                try {
                  const status = await analOS.getPageLoadStatus(tabId)
              ...
              if (!hasPlaceholder) {
                const injectQuery = async () => {
                  ...
                  try {
                    ...
                    const result = await analOS.executeJavaScript(tabId, script);
              ...
              if (provider.focusBeforeSubmit) {
                try {
                  await analOS.executeJavaScript(tabId, `
                    (function() {
            ...
            if (provider.autoSubmit && tabId != null) {
              if (hasPlaceholder || queryInjected) {
                ...
                await analOS.sendKeys(tabId, submitKey as chrome.analOS.Key)
              }
            }
          } catch (error) {
    ...
    }),
    {
      ...
      migrate: (persistedState, version) => {
        ...
        return {
          providers: DEFAULT_PROVIDERS,
          customProviders,
          selectedProviderId: legacyState.selectedProviderId || 'analos-agent',
          isDropdownOpen: false,
          enabledProviderIds: normalized.enabledProviderIds,
          disabledProviderIds: normalized.disabledProviderIds,
          hasLegacySynced: false
