...
import { AnalOSProvidersConfig, AnalOSProvider } from '@/lib/llm/settings/analOSTypes'
...
export function ChatInput({ isConnected, isProcessing }: ChatInputProps) {
  ...
  useEffect(() => {
    const computeOk = (cfg: AnalOSProvidersConfig) => {
    ...
    const handleWorkflow = (payload: any) => {
      ...
      if (payload && payload.data && payload.data.providersConfig) {
        computeOk(payload.data.providersConfig as AnalOSProvidersConfig)
      }
    }
  ...
  const isProviderUsable = (provider: AnalOSProvider | null): boolean => {
