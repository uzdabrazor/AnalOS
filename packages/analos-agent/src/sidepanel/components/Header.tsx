...
import { z } from 'zod'
import { AnalOSProvidersConfig, AnalOSProvidersConfigSchema } from '@/lib/llm/settings/types'
import { MCP_SERVERS, type MCPServerConfig } from '@/config/mcpServers'

const GITHUB_REPO_URL: string = 'https://github.com/analos-ai/AnalOS'
const DOCS_URL: string = 'https://docs.analos.com/'
...
export const Header = memo(function Header({ onReset, showReset, isProcessing, isTeachMode = false }: HeaderProps) {
  ...
  const [providersConfig, setProvidersConfig] = useState<AnalOSProvidersConfig | null>(null)
    ...
    try {
      ...
      try {
        await chrome.tabs.create({
          url: chrome.runtime.getURL('analos-settings.html')
        })
      } catch (fallbackError) {
    ...
    const handler = (payload: any) => {
      if (payload && payload.status === 'success' && payload.data) {
        ...
        if (payload.data.providersConfig) {
          try {
            const cfg = AnalOSProvidersConfigSchema.parse(payload.data.providersConfig)
