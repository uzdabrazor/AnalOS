...
export class FeatureFlags {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      Logging.log('FeatureFlags', `Initialized with AnalOS version: ${this.browserVersion}`)
    } catch (error) {
      Logging.log('FeatureFlags', `Failed to get AnalOS version, using defaults: ${error}`)
      this.initialized = true
    }
  }
