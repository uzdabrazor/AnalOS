...
export class Logging {
  ...
  /**
   * Log a metric event using the AnalOS metrics API with PostHog fallback
  ...
  public static async logMetric(eventName: string, properties?: Record<string, any>, sampling: number = 1.0): Promise<void> {
    ...
    try {
      await getAnalOSAdapter().logMetric(prefixedEventName, enhancedProperties)
    } catch (error) {
      // AnalOS failed, use PostHog fallback
      if (this.posthogApiKey && this.posthogInitialized) {
        try {
