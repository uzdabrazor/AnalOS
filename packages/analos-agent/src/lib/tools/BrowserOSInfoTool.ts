...
// Input schema for AnalOS info tool
const AnalOSInfoToolInputSchema = z.object({
  topic: z.enum([
    'overview',
    'installation',
    'configuration',
    ...
    'examples',
    'features'
  ]).describe('Information topic to retrieve about AnalOS'),
...
export type AnalOSInfoToolInput = z.infer<typeof AnalOSInfoToolInputSchema>

/**
 * AnalOSInfoTool - Provides comprehensive information about AnalOS features and usage
 * Contains various README-style documentation for different aspects of AnalOS
 */
export class AnalOSInfoToolImpl {
  ...
  async execute(input: AnalOSInfoToolInput): Promise<ToolOutput> {
    try {
      ...
      return toolError(`Failed to retrieve AnalOS info: ${error instanceof Error ? error.message : String(error)}`)
    }
  }

  private _getTopicInfo(topic: string, section?: string): string | null {
    const infoDatabase = {
      overview: `# AnalOS Overview

AnalOS is an AI-powered browser automation platform that enables intelligent web interaction through natural language commands.
...
- AI-assisted web navigation`,
      ...
      installation: `# AnalOS Installation Guide
...
git clone https://github.com/analos-ai/AnalOS-agent
cd AnalOS-agent
...
4. The AnalOS extension should now appear in your extensions

### 3. Configuration
1. Click the AnalOS extension icon
...
\`\`\``,
      ...
      configuration: `# AnalOS Configuration
...
\`\`\``,
      ...
      automation: `# AnalOS Automation Capabilities
...
\`\`\`javascript
// Execute a task
await analOS.execute("Navigate to Google and search for cats")

// Get browser state
const state = await analOS.getBrowserState()

// Take screenshot
const screenshot = await analOS.takeScreenshot()
\`\`\``,

      troubleshooting: `# AnalOS Troubleshooting
...
\`\`\`javascript
localStorage.setItem('analOS.debug', 'true')
\`\`\`
...
- Report issues with detailed reproduction steps`,
      ...
      examples: `# AnalOS Usage Examples
...
/**
 * Factory function to create AnalOSInfoTool for LangChain integration
 */
export function AnalOSInfoTool(executionContext: ExecutionContext): DynamicStructuredTool {
  const tool = new AnalOSInfoToolImpl(executionContext)

  return new DynamicStructuredTool({
    name: "analos_info_tool",
    description: `Get comprehensive information about AnalOS features, configuration, and usage.
    ...
    schema: AnalOSInfoToolInputSchema,
